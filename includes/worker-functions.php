...
function wStatusBadge($status) {
    $map = [
        ...
        'flagged'     => ['#f97316', '#ffedd5'],
    ];
    [$color, $bg] = $map[$status] ?? ['#6b7280', '#f3f4f6'];
    $label = ucwords(str_replace('_', ' ', $status));
    return "<span style='background:{$bg};color:{$color};padding:3px 12px;border-radius:20px;font-size:11px;font-weight:700;text-transform:capitalize;display:inline-block'>{$label}</span>";
}

function wMoney($amount) {
